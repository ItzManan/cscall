...
def build_utterance(idx: int, ex: dict, audio_dir: str) -> Utterance:
    """Map one Svarah record + its (already-written) wav path to an Utterance.

    Pure mapping (no IO) so it is unit-testable; the wav itself is written by the
    caller. Svarah is English, so cs_density/cs_bucket are fixed to 0 / "none".
    """
    uid = f"svarah_{idx:05d}"
    return Utterance(
        id=uid,
        audio_path=os.path.join(audio_dir, uid + ".wav"),
...
def main(argv=None) -> None:
    ...
    for i, ex in enumerate(ds):
        if args.limit and i >= args.limit:
            break
        u = build_utterance(i, ex, args.audio_dir)
        sf.write(u.audio_path, ex["audio"]["array"], 16000)
        rows.append(u)
        if (i + 1) % 100 == 0:
            print(f"  ...{i + 1} clips")
